...
typedef std::vector<TBL_STRING>::iterator ListTblStringIt;
typedef std::vector<TXT_STRING>::iterator ListTxtStringIt;
...
public:
	...
	unsigned int EncodeStream(const std::string& scriptbuf, unsigned int& BadCharOffset);
	//unsigned int EncodeStream(std::vector<char> scriptbuf);
	...
	std::vector<TBL_STRING>		StringTable;	// (Encoded) String table
	std::vector<TXT_STRING>		TxtStringTable; // Text String Table
	std::vector<std::string>	EndTokens;		// String end tokens

	//std::map<std::string, std::string> LookupText; // for looking up text values.  (dumping)
...
private:
	...
	//inline std::string& GetTextValue(std::string& Hexstring);
